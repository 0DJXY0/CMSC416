...
int main(int argc, char *argv[]) {
    ...
    /*
    if(myid ==3){                                                   
      MPI_Irecv(data1, Y_limit, MPI_INT, myid-1, 0, MPI_COMM_WORLD, &req);
      MPI_Wait(&req,&status);
    }
    else if(myid ==2){
      block[interval-1][67] = 9;
      MPI_Isend(block[interval-1], Y_limit, MPI_INT, myid+1, 0, MPI_COMM_WORLD, &req);
    }
    //MPI_Wait(&req,&status);
