...
int main(int argc, char *argv[]) {
	...
	if (data1[i]!=0){
	  printf("%d as %d\n",data1[i],i);}}
      MPI_Send(block[1], Y_limit, MPI_INT, myid+1, 0, MPI_COMM_WORLD);
    }
    else if(myid ==1){
      MPI_Recv(data1, Y_limit, MPI_INT, myid-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);    
